...
- Credential: prefer the passphrase-encrypted credential file created by `save-website-ftp-credential.ps1`. The credential file is portable to a new computer, but it requires the passphrase at upload time.
...
3. Run a dry run. If runtime credentials or an encrypted credential plus passphrase are available, the dry run compares remote file size, remote timestamp, and the remote deploy manifest hashes before listing upload candidates. Without credentials, dry run stays offline and reports the local inventory.
...
5. Upload only after the user has asked to deploy or clearly approved deployment. The upload script prompts for the credential-file passphrase when needed:
...
- Do not store the credential-file passphrase in the project. Prefer entering it at the upload prompt.
...
- `-CredentialPassphrase`: runtime-only passphrase alternative to the interactive prompt. Prefer the prompt so the passphrase is not left in shell history.
