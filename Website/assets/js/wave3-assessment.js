(function () {
  ...
  var audiences = {
    "aggregator": {
      ...
      alt: "Carrier network readiness desk with records and operating context",
      description: "Carrier networks, associations, freight aggregators, and organizations coordinating multiple operating entities.",
      categories: ["Network readiness", "Carrier governance", "Proof consistency", "Exception control", "Integration priority"],
      modules: ["Customer Demo - Business Operations", "BOF Vault", "Documents", "Policy Governance"],
      sections: [
        { name: "Network structure", questions: [
          ...
          ["network-owners", "Does each carrier or operating unit have an accountable owner for readiness follow-up?"],
          ["network-reporting", "Can leadership view network-level readiness without manually combining spreadsheets?"]
        ]},
        { name: "Carrier readiness", questions: [
          ["carrier-authority", "Are authority, insurance, and key carrier packet records reviewed before network use?"],
          ["driver-asset-readiness", "Can your network see driver and asset readiness before shared capacity is offered?"],
          ["subcontractor-records", "Are subcontractor records and operating requirements standardized across participants?"]
        ]},
        { name: "Shared capacity", questions: [
          ...
          ["capacity-constraints", "Are customer-specific constraints visible before a participating carrier is assigned?"]
        ]},
        { name: "Proof and exceptions", questions: [
          ["proof-standards", "Are standard proof requirements documented across carriers and customer programs?"],
          ["cross-carrier-access", "Can authorized staff access cross-carrier documents and history when exceptions occur?"],
          ["exception-escalation", "Do cross-carrier exceptions have owners, due dates, and customer impact notes?"],
    ...
    "private-fleet": {
      ...
      categories: ["Workforce readiness", "Operational control", "Fleet safety", "Administrative control", "Executive visibility"],
      ...
      sections: [
        { name: "Workforce and driver readiness", questions: [
          ...
          ["onboarding", "Is onboarding tied to required documents, training, policy acknowledgments, and first assignment release?"],
        ...
        { name: "Safety and maintenance", questions: [
          ...
          ["safety-exceptions", "Do safety exceptions create clear holds or review requirements before movement?"]
        ]},
        { name: "Business operations and leadership", questions: [
          ...
          ["vendor-records", "Are vendor, insurance, and administrative records assigned to owners and review dates?"],
    ...
    "for-hire-fleet": {
      ...
      modules: ["Dispatch & Operations", "Documents", "Settlements & Billing", "BOF Vault"],
      sections: [
        { name: "Customer and load intake", questions: [
          ["rate-confirmation", "Are rate confirmations and customer requirements captured before dispatch release?"],
        ...
        { name: "Proof and claims", questions: [
          ...
          ["claims-evidence", "Can claims evidence and incident notes be found without rebuilding the packet?"]
        ]},
        { name: "Settlements and billing", questions: [
          ...
          ["settlement-holds", "Are settlement holds tied to load proof, driver pay, deductions, and approvals?"],
    ...
    "government": {
      ...
      sections: [
        ...
        { name: "Procurement and vendor readiness", questions: [
          ["vendor-records", "Are vendor, contract, COI, and procurement records current and assigned to owners?"],
          ["grant-documentation", "Are grant, funding, and purchase records retained with review history?"],
          ["procurement-status", "Can staff see procurement blockers before they affect fleet readiness?"]
        ]},
        ...
        { name: "Policy and audit controls", questions: [
          ["policy-versions", "Are policy versions, acknowledgments, training status, and exceptions controlled?"],
          ["public-records", "Can public-record-ready documents be located without recreating history?"],
          ["continuity-planning", "Are continuity plans tied to owners, readiness evidence, and review rhythm?"]
        ]}
      ]
    },
    "driver": {
  ...
  var answers = {};
  var state = { type: getTypeFromUrl(), step: "landing", index: 0 };
  ...
  function renderLanding() {
    root.innerHTML = [
      '<div class="wave3-card-grid">',
      order.map(function (key) {
        var audience = audiences[key];
        return '<article class="wave3-assessment-card">' +
          '<img src="' + audience.image + '" alt="' + escapeHtml(audience.alt) + '" style="display:block;height:190px;object-fit:cover;width:100%;">' +
          ...
          '<div class="wave3-card-actions"><button type="button" data-select-audience="' + key + '">Start ' + escapeHtml(audience.title) + '</button><a href="' + audience.route + '">Audience page</a></div>' +
          '</div></article>';
      }).join(""),
      '</div>'
    ].join("");
  ...
  function renderAssessment() {
    ...
    root.innerHTML = [
      '<div class="wave3-assessment-shell">',
        '<div class="wave3-assessment-toolbar">',
          '<div><p class="wave3-eyebrow">' + escapeHtml(audience.eyebrow) + '</p><h3>' + escapeHtml(audience.title) + '</h3><p>' + escapeHtml(audience.description) + '</p></div>',
          '<div class="wave3-assessment-tabs" role="tablist" aria-label="Assessment audience paths">',
            order.map(function (key) {
              return '<button class="wave3-assessment-tab" type="button" role="tab" aria-selected="' + (key === state.type ? "true" : "false") + '" data-select-audience="' + key + '">' + escapeHtml(audiences[key].title) + '</button>';
            }).join(""),
          '</div>',
        '</div>',
        '<div class="wave3-assessment-stage">',
          '<div>',
            '<div class="wave3-progress" aria-live="polite"><strong>Section ' + (current.sectionIndex + 1) + ' of ' + audience.sections.length + ': ' + escapeHtml(section.name) + '</strong><span> Question ' + (state.index + 1) + ' of ' + list.length + ' / ' + complete.answered + ' answered</span><div class="wave3-progress-bar" aria-hidden="true"><span style="width:' + Math.max(4, Math.round(((state.index + 1) / list.length) * 100)) + '%"></span></div></div>',
            ...
            '</form>',
          '</div>',
          '<aside class="wave3-result-card" aria-label="Assessment context">',
            '<span>Current path</span><h3>' + escapeHtml(audience.title) + '</h3>',
            '<p>Answers are kept in browser memory only for this page session. Switching audiences preserves current answers until the page reloads.</p>',
            '<div class="wave3-card-actions"><a href="' + audience.route + '">Return to audience page</a><button type="button" data-reset-audience>Restart path</button></div>',
          '</aside>',
        '</div>',
      '</div>'
    ].join("");
  ...
  function renderReview() {
    var audience = audiences[state.type];
    var list = questionList(audience);
    root.innerHTML = '<div class="wave3-question-card"><p class="wave3-eyebrow">Review answers</p><h3>' + escapeHtml(audience.title) + '</h3><ul class="wave3-review-list">' +
      ...
      '</ul><div class="wave3-actions"><button class="wave3-button wave3-button--secondary" type="button" data-back-to-questions>Back to Questions</button><button class="wave3-button wave3-button--primary" type="button" data-calculate-results>Calculate Result</button></div></div>';
  }
  ...
  function calculate(type) {
    ...
    return { pct: pct, band: band, gaps: gaps, sections: sectionScores };
  }
  ...
  function renderResults() {
    ...
    root.innerHTML = [
      '<div class="wave3-question-card" aria-live="polite">',
        '<p class="wave3-eyebrow">Assessment result</p>',
        '<h3>' + escapeHtml(result.band) + '</h3>',
        '<p>Overall synthetic readiness score: ' + result.pct + '%. This score is directional and based only on the answers entered in this browser session.</p>',
        ...
        '<div class="wave3-card-actions">' + audience.modules.map(function (module) { return '<span>' + escapeHtml(module) + '</span>'; }).join("") + '</div>',
        ...
        '<div class="wave3-actions"><a class="wave3-button wave3-button--primary" href="' + audience.route + '">' + escapeHtml(audience.cta) + '</a>' +
          ...
          '<button class="wave3-button wave3-button--secondary" type="button" data-reset-audience>Restart Assessment</button></div>',
      '</div>'
    ].join("");
  }
  ...
  root.addEventListener("change", function (event) {
    if (event.target && event.target.name === "answer") {
  ...
  root.addEventListener("click", function (event) {
    ...
    if (select) {
      setType(select.getAttribute("data-select-audience"), true);
      return;
    }
