(function () {
  var DATA_URL = "/assets/data/ascendtms-mock-loads.json";
  ...
  function matchAscendDriverToBofDriver(load, data) {
    var driver = null;
    var method = "none";
    var confidence = "none";
    ...
    return {
      ...
      driver: driver,
      ascendDriverName: load.assignedDriverName,
  ...
  function mapAscendLoadToBofReview(load, data) {
    var match = matchAscendDriverToBofDriver(load, data);
    var review = {
      load: load,
      driverMatch: match,
  ...
  function createSimulatedHandoffPayload(decision, load) {
    return {
      ascendLoadId: load.ascendLoadId,
  ...
  function setText(root, name, value) {
    var node = root.querySelector('[data-ascend-target="' + name + '"]');
  ...
  function renderReview(root, data, loadId) {
    var load = data.loads.filter(function (item) {
      return item.ascendLoadId === loadId;
    })[0] || data.loads[0];
    var review = mapAscendLoadToBofReview(load, data);
    var decision = review.releaseDecision;
    var handoffPayload = createSimulatedHandoffPayload(decision, load);

    root.querySelectorAll("[data-ascend-load]").forEach(function (button) {
      button.classList.toggle("is-active", button.getAttribute("data-ascend-load") === load.ascendLoadId);
      button.setAttribute("aria-pressed", String(button.getAttribute("data-ascend-load") === load.ascendLoadId));
    });

    setText(root, "ascendLoadId", load.ascendLoadId);
    ...
    setText(root, "ascendStatus", formatStatus(load.loadStatus));
  ...
  function initWorkflow(root, data) {
    var selector = root.querySelector("[data-ascend-load-selector]");
    if (selector) {
      selector.innerHTML = data.loads.map(function (load, index) {
        return '<button type="button" data-ascend-load="' + escapeHtml(load.ascendLoadId) + '" aria-pressed="' + (index === 0 ? "true" : "false") + '">' +
          '<span>' + escapeHtml(load.ascendLoadId) + '</span>' +
          '<strong>' + escapeHtml(load.customerName) + '</strong>' +
          '<em>' + escapeHtml(load.assignedDriverName) + '</em>' +
          '</button>';
      }).join("");
      selector.querySelectorAll("[data-ascend-load]").forEach(function (button) {
        button.addEventListener("click", function () {
          renderReview(root, data, button.getAttribute("data-ascend-load"));
        });
      });
    }
    renderReview(root, data, data.loads[0].ascendLoadId);
  }

  window.AscendTmsIntegration = {
    fetchAscendLoads: function () {
      return loadData().then(function (data) { return data.loads; });
    },
    fetchAscendLoadById: function (ascendLoadId) {
      return loadData().then(function (data) {
        return data.loads.filter(function (load) { return load.ascendLoadId === ascendLoadId; })[0] || null;
      });
    },
    fetchAscendDocumentsForLoad: function (ascendLoadId) {
      return window.AscendTmsIntegration.fetchAscendLoadById(ascendLoadId).then(function (load) {
    ...
    mapAscendLoadToBofReview: function (load) {
      return loadData().then(function (data) {
        return mapAscendLoadToBofReview(load, data);
      });
    }
  };

  var workflowRoot = document.querySelector("[data-ascend-workflow]");
  if (workflowRoot) {
