(function () {
  ...
  if (nav) {
    ...
    function clearNavCloseTimer(menu) {
      var timer = navCloseTimers.get(menu);
      if (timer) window.clearTimeout(timer);
      navCloseTimers.delete(menu);
    }

    function closeWhenMenuIsNoLongerActive(menu) {
      clearNavCloseTimer(menu);
      navCloseTimers.set(menu, window.setTimeout(function () {
        if (!menu.matches(":hover") && !menu.matches(":focus-within")) {
    ...
    navMenus.forEach(function (menu) {
      var linkCount = menu.querySelectorAll(".nav-dropdown a").length;
      if (linkCount >= 8) menu.classList.add("nav-menu--dense");

      menu.addEventListener("focusin", function () {
        clearNavCloseTimer(menu);
        closeNavMenus(menu);
        setNavMenu(menu, true);
      });

      menu.addEventListener("focusout", function () {
        closeWhenMenuIsNoLongerActive(menu);
      });
  ...
  if (interactiveDemo) {
    ...
    var driverDocumentVault = [
      {
        ...
        artifact: {
          image: "/assets/images/documents/drivers/licenses/license-drv-001.png?v=3",
    ...
    interactiveDemo.querySelectorAll("[data-demo-action]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        if (action === "enter" || action === "enter-record") {
          var entryRoute = action === "enter" ? "/interactive-demo/loading/" : "/interactive-demo/";
          window.location.href = window.bofPath ? window.bofPath(entryRoute) : entryRoute;
          return;
        }
        if (action === "reset") {
          appState.selectedLoad = "tms-ld-10482";
        ...
        if (action === "reset-session") {
          var resetRoute = "/interactive-demo/loading/";
          window.location.href = window.bofPath ? window.bofPath(resetRoute) : resetRoute;
          return;
        }
    ...
    interactiveDemo.addEventListener("click", function (event) {
      ...
      if (driverPage) {
        window.location.href = window.bofPath ? window.bofPath(driverPage) : driverPage;
        return;
      }
