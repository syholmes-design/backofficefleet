(function () {
  ...
  function ensurePreTripProofPanel(scene) {
    if (!scene || scene.getAttribute("data-scene") !== "pre-trip-readiness") return;
    if (scene.querySelector("img[src*='freightbrace-cargo-securement-proof']")) return;

    var visual = scene.querySelector(".ad-visual");
    if (!visual) {
      visual = document.createElement("div");
      visual.className = "ad-visual";
      scene.appendChild(visual);
    }

    visual.innerHTML = '<div class="ad-panel">' +
      '<div class="ad-panel-head"><h3>Readiness review</h3><span>Pre-trip control</span></div>' +
      '<article class="ad-proof" style="--i:2"><img src="/assets/images/photos/site-pass/freightbrace-cargo-securement-proof.webp" alt="Cargo securement barrier protecting reels inside a trailer"><div><h3>Cargo securement proof</h3><p>FreightBrace-style load protection photo is attached before the trip moves forward.</p></div></article>' +
      '<div class="ad-readiness-list">' +
      '<div class="ad-readiness-row"><strong>Driver qualification</strong><span class="status ready">Ready</span></div>' +
      '<div class="ad-readiness-row"><strong>CDL verification</strong><span class="status ready">Ready</span></div>' +
      '<div class="ad-readiness-row"><strong>Medical card</strong><span class="status review">At Risk</span></div>' +
      '<div class="ad-readiness-row"><strong>Training</strong><span class="status ready">Ready</span></div>' +
      '<div class="ad-readiness-row"><strong>Equipment readiness</strong><span class="status ready">Ready</span></div>' +
      '<div class="ad-readiness-row"><strong>Cargo securement photo</strong><span class="status ready">Ready</span></div>' +
      '<div class="ad-readiness-row"><strong>Insurance verification</strong><span class="status blocked">Blocked</span></div>' +
      '<div class="ad-readiness-row"><strong>Document readiness</strong><span class="status review">At Risk</span></div>' +
      '</div></div>';
  }
  ...
  function activateScene(index, reason) {
    ...
    ensurePreTripProofPanel(scenes[activeIndex]);
  ...
  function getInitialSceneIndex() {
    var requestedScene = "";
    if (root.dataset.initialScene) {
      requestedScene = root.dataset.initialScene;
    } else if (window.location.hash) {
      requestedScene = window.location.hash.replace(/^#/, "");
    }

    if (requestedScene) {
      var requestedIndex = scenes.findIndex(function (scene) {
        return scene.getAttribute("data-scene") === requestedScene;
      });
      if (requestedIndex >= 0) return requestedIndex;
    }

    var activeSceneIndex = scenes.findIndex(function (scene) {
      return scene.classList.contains("is-active");
    });
    return activeSceneIndex >= 0 ? activeSceneIndex : 0;
  }
  ...
  activateScene(getInitialSceneIndex());
