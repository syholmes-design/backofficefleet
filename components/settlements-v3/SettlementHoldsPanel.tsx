...
import { AlertTriangle, DollarSign, Clock, CheckCircle, User, FileText, Truck, Shield, Wrench, Calendar } from "lucide-react";
...
export function SettlementHoldsPanel({ weekEnding, driverId }: SettlementHoldsPanelProps) {
  const [holds, setHolds] = useState<SettlementHold[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Settlement hold loader is a local async routine; rerun only when filters change.
  useEffect(() => {
    loadHolds();
