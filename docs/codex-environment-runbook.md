...
- This project folder is intentionally shared through the cloud; do not treat that as a blocker by itself.
- Ensure all files are fully hydrated locally before running Node commands.
- Close Office documents or other tools that may leave `.~lock.*#` files when scripts need to read those files.
...
Recommended in-place reset for the shared project folder:
...
If Node reports `UNKNOWN: unknown error, read`, stay in the shared folder and check for cloud placeholder files, file locks, interrupted dependency installs, and stale build caches before rerunning the command.
