...
// Verification script for settlement deductions reconciliation
import { loadBofData } from '../lib/load-bof-data.ts';
...
async function main() {
  ...
  try {
    ...
    for (const settlement of settlements) {
      const driver = data.drivers.find(d => d.id === settlement.driverId);
      ...
      const difference = Math.abs(settlement.totalDeductions - visibleTotal);
