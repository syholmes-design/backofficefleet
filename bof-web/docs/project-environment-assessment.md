...
Because `node -e` and direct package metadata reads worked, the issue may be intermittent or file-specific rather than a universal Node failure. Given the project lives under OneDrive and includes thousands of generated/public files, likely suspects include cloud-file hydration, OneDrive file locking/sync state, or partially available placeholder files. An assessor should verify this outside OneDrive or with all files pinned locally.

## Environment Optimization Recommendations

For Codex and CI reliability, the highest-value environment changes are:

1. Move or clone the project into a non-OneDrive local development path, such as `C:\dev\bof-web`.
2. Ensure all files are fully hydrated locally before running Node scripts.
3. Reinstall dependencies from scratch after moving the project:
   - delete `node_modules`
   - delete `.next`
   - run `npm ci`
...
- Resolve OneDrive or filesystem read instability.
