...
export default function RfActionsPage() {
  ...
  const actions = buildRfActions(data);

  return (
    <div className="bof-page">
      <h1 className="bof-title">RF action engine</h1>
      <p className="bof-lead">
        Actions generated from the load proof layer — payment blocks, missing
        proofs, and dispute exposure. Same data powers Settlements and Money at
        Risk. The{" "}
        <Link href="/loads" className="bof-link-secondary">
          document automation engine
        </Link>{" "}
        on each load merges these RF actions with RFID dock/fuel/maintenance
        snapshots (see RF POD, seal, fuel, and maintenance reports).
      </p>
