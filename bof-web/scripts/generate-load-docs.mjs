...
function deriveLoadContext(data, load, now = new Date()) {
  ...
  return {
    ...
    nmfcClass: "NMFC 156600 / Class 70 ",
    rate: money(rate),
    ...
    remittanceNote: "Remit to BackOfficeFleet Clearing · ACH Ref: BOF-OPS",
    ...
    if (load.status === "Delivered") {
      ...
      if (factoringPacketTemplate) {
        const factoringPacketData = {
          ...
          invoiceNumber: `INV-${ctx.generatedAt.replace(/[-:TZ.]/g, "").slice(0, 8)}-${ctx.loadId.slice(-3)}`,
          invoiceAmount: load.revenue || "0.00",
          deliveryDate: load.deliveryAt || load.deliveryDate || "Unknown",
          ...
          preparedDate: ctx.generatedDate,
          reviewedBy: "Finance Manager",
          reviewDate: ctx.generatedDate,
