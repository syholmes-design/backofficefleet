...
try {
  ...
  console.log(`DEBUG: Reading from ${XLSX_PATH}`);
  ...
  console.log(`\nDEBUG: After merge - checking first driver:`);
  ...
  console.log(`\nDEBUG: After patchDriversForJohnCarter - checking first driver:`);
  ...
  console.log(`\nDEBUG: After augmentDriversWithFleetDemoFields - checking first driver:`);
  ...
  // Check if Master Data fields are present
  ...
  console.log(`  Has secondary data: ${hasSecondaryData}`);
  console.log(`  Driver object keys: ${Object.keys(augmentedDriver).filter(k => k.includes('Emergency')).join(', ')}`);
  
  if (!hasPrimaryData || !hasSecondaryData) {
    console.log(`\n❌ ISSUE: Master Data lost during build process`);
    console.log(`   The drivers object at the end of build doesn't have Master Data fields`);
  } else {
    console.log(`\n✅ SUCCESS: Master Data preserved through build process`);
    console.log(`   The issue must be in the file write process`);
  }
