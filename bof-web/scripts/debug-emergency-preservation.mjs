...
try {
  ...
  // Show first few drivers with emergency data from v2 Excel
  console.log("\nFirst 3 drivers with v2 Excel emergency data:");
  drivers.slice(0, 3).forEach(driver => {
    ...
    console.log(`    Has all emergency data: ${driver.emergencyContactName && driver.emergencyContactRelationship && driver.emergencyContactPhone}`);
  });
  ...
  drivers.slice(0, 3).forEach(driver => {
    ...
    const hasEmergencyName = driver.emergencyContactName && driver.emergencyContactName.trim() !== '';
    const hasEmergencyRelation = driver.emergencyContactRelationship && driver.emergencyContactRelationship.trim() !== '';
    const hasEmergencyPhone = driver.emergencyContactPhone && driver.emergencyContactPhone.trim() !== '';
    const hasRealEmergencyData = hasEmergencyName && hasEmergencyRelation && hasEmergencyPhone;
    const isDemoData = driver.emergencyContactName && (
      driver.emergencyContactName.includes('demo') || 
      driver.emergencyContactName.includes('— emergency contact')
    );
    
    console.log(`    hasRealEmergencyData: ${hasRealEmergencyData}, isDemoData: ${isDemoData}`);
    console.log(`    Should preserve: ${hasRealEmergencyData && !isDemoData}`);
  });
