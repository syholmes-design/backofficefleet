...
## Shared Intelligence Budget Rule

This shared BackOfficeFleet project runs in low-effort mode by default on every Codex instance.

Activate `.codex/agents/intelligence-budget-controller.md` for every task. Codex should not voluntarily escalate to medium or high reasoning.

Use the shortest useful response and avoid generic AI-style explanations.

Do not spend usage on:

- broad best-practice essays;
- repeated advice;
- copied-looking ChatGPT/Copilot output;
- line-by-line review of generic pasted material;
- unnecessary new personas;
- large reports when a short decision is enough.

Longer reasoning is allowed only when the user explicitly asks for it.

Low-effort mode must not suppress safety warnings about overwrites, rollback risk, backup failure, change-memory protection, destructive edits, source-of-truth conflicts, validation integrity, or obvious demo-breaking changes.
