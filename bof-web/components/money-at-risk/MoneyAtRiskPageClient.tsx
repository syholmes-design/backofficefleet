...
function moneyAtRiskSummary(data: BofData) {
  return "moneyAtRiskSummary" in data && data.moneyAtRiskSummary ? data.moneyAtRiskSummary : null;
}

function moneyAtRiskRows(data: BofData) {
  return "moneyAtRisk" in data && Array.isArray(data.moneyAtRisk) ? data.moneyAtRisk : [];
}

export function MoneyAtRiskPageClient() {
  const { data } = useBofDemoData();

  const summary = useMemo(() => moneyAtRiskSummary(data), [data]);
  const rows = useMemo(() => moneyAtRiskRows(data), [data]);
  const proofRows = useMemo(() => buildProofRiskRows(data), [data]);
  const storyRows = useMemo(() => buildMoneyStoryRows(data), [data]);

  return (
    <div className="bof-page">
      <h1 className="bof-title">Money at Risk</h1>
      <p className="bof-lead">Executive financial exposure — same figures inform Command Center.</p>
      ...
      {summary && (
        <section className="bof-kpi-grid" aria-label="Risk rollups">
          <div className="bof-kpi">
            ...
            <span className="bof-kpi-value">{formatUsdFull(summary.totalAtRisk)}</span>
          </div>
          <div className="bof-kpi">
            <span className="bof-kpi-label">Payroll pending amount</span>
            <span className="bof-kpi-value">{formatUsdFull(summary.payrollPending)}</span>
          </div>
          <div className="bof-kpi">
            <span className="bof-kpi-label">Settlement holds</span>
            <span className="bof-kpi-value">{formatUsdFull(summary.settlementHolds)}</span>
          </div>
          <div className="bof-kpi">
            <span className="bof-kpi-label">Claims / damage exposure</span>
            <span className="bof-kpi-value">{formatUsdFull(summary.claimsExposure)}</span>
          </div>
          <div className="bof-kpi">
            <span className="bof-kpi-label">Compliance-driven risk</span>
            <span className="bof-kpi-value">{formatUsdFull(summary.complianceRisk)}</span>
          </div>
          <div className="bof-kpi">
            <span className="bof-kpi-label">Maintenance-driven risk</span>
            <span className="bof-kpi-value">{formatUsdFull(summary.maintenanceRisk)}</span>
          </div>
        </section>
      )}
      ...
      {proofRows.length > 0 && (
        <section className="bof-section" aria-label="Proof-driven exposure">
          ...
          <p className="bof-muted bof-small">
            Derived from missing or blocking proof items on each load. Open the load to close gaps.
          </p>
              ...
              <tbody>
                {proofRows.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td>
                      <Link href={`/loads/${row.loadId}#document-engine`} className="bof-driver-link">
                        {row.loadId}
                      </Link>
                      <div>
      ...
      {storyRows.length > 0 && (
        <section className="bof-section" aria-label="Claim and RFID financial story">
          <h2 className="bof-h2">Claim, proof, and RFID — financial story</h2>
          <p className="bof-muted bof-small">
            What is wrong, how much is affected, and what BOF recommends next — derived from claim
            workspaces, proof holds, and RFID validation lanes (demo estimates).
          </p>
          <div className="bof-table-wrap">
            <table className="bof-table">
              <thead>
              ...
              <tbody>
                {storyRows.map((s) => (
                  <tr key={s.id}>
                    <td>
                      <span className="bof-badge bof-badge-neutral">{s.lane.replace(/_/g, " ")}</span>
                    </td>
                    <td className="bof-small">{s.headline}</td>
                    <td className="bof-table-photo-cell">
                      {s.driverId ? (
                        <DriverLink driverId={s.driverId} className="bof-table-driver-hit">
                          <DriverAvatar
                            name={s.driverName}
                    ...
                    <td>
                      {s.driverId ? (
                        <DriverLink driverId={s.driverId}>{s.driverName}</DriverLink>
                      ) : (
                        <span className="bof-muted">—</span>
                      )}
                    </td>
                    ...
                    <td className="bof-small">
                      {s.loadId ? (
                        <Link href={`/loads/${s.loadId}#document-engine`} className="bof-link-secondary">
                          Engine
                        </Link>
                      ) : (
                        "—"
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id}>
                ...
                <td>{row.owner}</td>
                <td>
                  <span className="bof-badge bof-badge-neutral">{row.status}</span>
                </td>
                <td className="bof-small">
                  <a
