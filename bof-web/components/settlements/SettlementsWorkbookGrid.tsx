...
export function SettlementsWorkbookGrid() {
  ...
  return (
    <div className="bof-page">
      <p className="bof-lead mb-4 text-sm">
        <Link href="/settlements" className="bof-link-secondary">
          ← Payroll operations
        </Link>
      </p>
      ...
      <div className="bof-settlements-wrap">
        <table className="bof-table bof-table-compact bof-settlements-table">
          ...
          <tbody>
            {rows.map((row) => (
              <SettlementTableRow key={row.driverId} row={row} data={data} />
            ))}
          </tbody>
        </table>
      </div>
