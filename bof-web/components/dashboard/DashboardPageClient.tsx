...
export function DashboardPageClient() {
  ...
  const attention = useMemo(() => buildCommandCenterItems(data).slice(0, 6), [data]);

  const loadsActive = useMemo(
    () => data.loads.filter((l) => l.status === "En Route" || l.status === "Pending").length,
    [data.loads]
  );
  const openCompliance = useMemo(
    () => data.complianceIncidents.filter((c) => c.status === "OPEN").length,
    [data.complianceIncidents]
  );

  return (
    <div className="bof-page">
      <h1 className="bof-title">Dashboard</h1>
      <p className="bof-lead">Overview only — open modules for full detail.</p>
      ...
      <section className="bof-section" aria-label="Modules">
        ...
        <div className="bof-cards">
          ...
          <Link href="/settlements" className="bof-card">
            ...
            <span className="bof-card-desc">
              Net {formatUsd(st.totalNet)} this period · {st.pendingOrHold} awaiting action
            </span>
          </Link>
          <Link href="/money-at-risk" className="bof-card">
            <span className="bof-card-title">Money at Risk</span>
            <span className="bof-card-desc">Financial exposure register and rollups</span>
          </Link>
          <Link href="/command-center" className="bof-card">
            <span className="bof-card-title">Command Center</span>
            <span className="bof-card-desc">Cross-functional issues and next actions</span>
          </Link>
          <Link href="/drivers" className="bof-card">
            <span className="bof-card-title">Drivers</span>
            <span className="bof-card-desc">Profiles, readiness, and credential documents</span>
          </Link>
          <Link href="/loads" className="bof-card">
            <span className="bof-card-title">Loads / dispatch</span>
            <span className="bof-card-desc">Loads with links to assigned drivers</span>
          </Link>
        </div>
      </section>
        ...
        <ul className="bof-attention-list">
          {attention.map((item) => (
            <li key={item.id} className="bof-attention-item">
              <span className={`bof-sev bof-sev-${item.severity}`}>{item.severity}</span>
              <div>
                  ...
                  {item.driver && item.driverId ? (
                    <>
                      ...
                      <span className="bof-attention-driver">
                        <DriverLink driverId={item.driverId} className="bof-attention-driver-photo">
                          <DriverAvatar
                            name={item.driver}
