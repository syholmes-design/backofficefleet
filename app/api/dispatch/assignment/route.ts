...
import { createAssignment } from "@/lib/services/dispatchAssignmentService";
...
function getOptionalString(body: Record<string, unknown>, fieldName: string) {
  ...
  if (typeof value !== "string" || value.trim().length === 0) {
    throw Object.assign(new Error(`${fieldName} must be a non-empty string when provided`), { statusCode: 422 });
  }
  return value.trim();
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    assertAllowedFields(body, ["loadId", "driverId", "tractorId", "trailerId"]);

    const assignment = await createAssignment(
      getSessionUser(session),
      getRequiredString(body, "loadId"),
      getRequiredString(body, "driverId"),
      getRequiredString(body, "tractorId"),
      getOptionalString(body, "trailerId"),
    );
