...
import { listReleaseHistory } from "@/lib/services/dispatchReleaseService";
...
export async function GET(_request: NextRequest, context: { params: Promise<{ loadId: string }> }) {
  ...
  try {
    const releaseHistory = await listReleaseHistory(getSessionUser(session), loadId);
    return NextResponse.json(releaseHistory);
  } catch (error) {
