...
import { getCurrentReadinessForLoad } from "@/lib/services/dispatchReleaseService";
...
export async function GET(_request: NextRequest, context: { params: Promise<{ loadId: string }> }) {
  ...
  try {
    const readiness = await getCurrentReadinessForLoad(getSessionUser(session), loadId);
