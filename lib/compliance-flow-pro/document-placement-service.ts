  ...
  private async generateMissing(action: PlacementAction): Promise<PlacementActionResult> {
    ...
    for (const file of files) {
      ...
      try {
        ...
        await this.generateMissingDocumentFile(fileRecord.documentType, targetPath);
...
<body>
  ...
  /**
   * Generate a demo-safe missing-document file.
   */
  private async generateMissingDocumentFile(documentType: string, targetPath: string): Promise<void> {
    const documentContent = this.generateMissingDocumentTemplate(documentType);
    await fs.writeFile(targetPath, documentContent, 'utf8');
  }

  /**
   * Generate missing-document template.
   */
  private generateMissingDocumentTemplate(documentType: string): string {
    const templates: Record<string, string> = {
      "CDL": `
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .missing-document-callout { text-align: center; padding: 40px; background: #fff3cd; border-radius: 4px; margin: 20px 0; }
...
<body>
    <div class="container">
        ...
        <div class="missing-document-callout">
